...
export default function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 flex justify-center px-4 pt-4 sm:px-6">
      <motion.div
        initial={{ y: -24, opacity: 0, maxWidth: 1240 }}
        animate={{
          y: 0,
          opacity: 1,
        ...
        className={`flex w-full items-center justify-between rounded-full border px-4 backdrop-blur-xl sm:px-6 ${
          scrolled ? "border-ink/6" : "border-transparent"
        }`}
      >
        {/* Logo */}
        <a href="#home" className="flex items-center gap-2.5">
          <AhwMark className="h-9 w-9" />
        ...
        <nav className="hidden items-center gap-1 lg:flex">
          {navLinks.map((link) => (
            ...
            >
              {link.label}
              <span className="absolute inset-x-4 bottom-1 h-px origin-left scale-x-0 bg-brand-red transition-transform duration-300 ease-nav group-hover:scale-x-100" />
            </a>
          ))}
        </nav>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute left-4 right-4 top-[72px] z-40 overflow-hidden rounded-3xl border border-ink/6 bg-paper/90 p-3 shadow-glass-lg backdrop-blur-xl sm:left-6 sm:right-6 lg:hidden"
          >
