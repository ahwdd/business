      ...
      <div className="mx-auto max-w-[1240px] px-6">
        ...
        <div className="relative grid gap-5 sm:grid-cols-2 lg:grid-cols-3 lg:grid-rows-3">
          ...
          {ecosystemBrands.map((brand, i) => {
            const Icon = brandIcons[brand.name];
            ...
            return (
              <motion.a
                ...
                className={`group relative z-10 flex min-h-[180px] flex-col justify-between overflow-hidden rounded-card border p-6 transition-all duration-400 ${
                  isCenter
                    ? "border-brand-red/25 bg-paper shadow-red-glow"
                    : "border-ink/[0.06] bg-mist hover:bg-paper hover:shadow-glass-lg"
                } ${SPOKE_POSITION[i]} ${
              ...
              >
                <div
                  className={`absolute -right-8 -top-8 h-28 w-28 rounded-full blur-2xl transition-transform duration-500 group-hover:scale-125 ${
                    isCenter ? "bg-brand-red/10" : "bg-ink/[0.04]"
                  }`}
                />
                ...
                <div className="relative flex items-center justify-between">
                  <span
                    className={`flex h-11 w-11 items-center justify-center rounded-xl p-2 ${
                      isCenter ? "bg-brand-red text-white" : "bg-ink text-paper"
                    }`}
                  >
