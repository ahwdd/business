...
export default function Hero() {
  return (
    <section
      id="home"
      className="relative flex min-h-[calc(100vh-var(--nav-height))] items-center overflow-hidden bg-paper pt-[var(--nav-height)] pb-16 sm:pb-20"
    >
      {/* Background layers */}
      ...
      <motion.div
        ...
        className="absolute -left-32 top-1/4 h-[420px] w-[420px] rounded-full bg-brand-red/[0.07] blur-[120px]"
      />
      <motion.div
        animate={{ opacity: [0.6, 0.3, 0.6] }}
        transition={{ duration: 12, repeat: Infinity, ease: "easeInOut", delay: 2 }}
        className="absolute -right-24 bottom-0 h-[360px] w-[360px] rounded-full bg-ink/[0.04] blur-[120px]"
      />
        ...
        <div className="grid items-center gap-14 lg:grid-cols-[1.15fr_0.85fr]">
          ...
          >
            ...
            <motion.div
              ...
              className="absolute -right-6 -top-8 z-0 hidden h-24 w-24 rounded-3xl border border-ink/[0.06] bg-paper/70 shadow-glass backdrop-blur-md sm:block"
            />
            <motion.div
              animate={{ y: [0, 16, 0], x: [0, -8, 0], rotate: [0, -3, 0] }}
              transition={floatSlower}
              className="absolute -bottom-7 -left-6 z-20 hidden h-16 w-16 rounded-2xl border border-brand-red/20 bg-brand-red/[0.06] backdrop-blur-md sm:block"
            />

            <div className="glass-panel relative z-10 overflow-hidden rounded-card-lg p-7 shadow-glass-lg sm:p-9">
              <div className="absolute -right-10 -top-10 h-40 w-40 rounded-full bg-brand-red/[0.08] blur-3xl" />
              ...
              <div className="grid grid-cols-2 gap-5">
                {heroStats.map((stat, i) => (
                  <motion.div
                    ...
                    className="rounded-2xl border border-ink/[0.05] bg-paper/70 p-5"
                  >
            ...
            <motion.div
              ...
              className="absolute -right-5 bottom-10 z-20 hidden h-12 w-12 rounded-2xl border border-ink/[0.06] bg-paper/80 shadow-glass backdrop-blur-md lg:block"
            />
          </motion.div>
        </div>
      </div>
