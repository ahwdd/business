  ...
  return (
    <section id="partners" className="relative overflow-hidden bg-mist py-24 sm:py-32">
      ...
      <div className="relative flex flex-col gap-4">
        <div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-24 bg-linear-to-r from-mist to-transparent sm:w-40" />
        <div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-24 bg-linear-to-l from-mist to-transparent sm:w-40" />
    ...
    <div className="flex overflow-hidden">
      ...
      >
        {loop.map((name, i) => (
          <span
            key={`${name}-${i}`}
            className="flex items-center whitespace-nowrap rounded-full border border-ink/6 bg-paper px-6 py-3 text-sm font-semibold tracking-wide text-body/70 transition-colors hover:text-ink"
          >
            {name}
          </span>
        ))}
